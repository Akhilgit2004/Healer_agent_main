...
✨ Key Features
Polyglot Repair Engine: Supports Python, JavaScript (Node.js), and C++ out of the box.
...
Local LLM: Pull the required model (default is Llama 3):

Bash
ollama pull llama3
System Compilers: (Optional, for verification)
...
Bash
git clone https://github.com/Akhilgit2004/Healer_agent_main.git
...
Analysis: The agent parses the provided log file using Regex to find filenames and error types.

Context Loading: It reads the target source code and queries the Vector Database for similar past bugs.

LLM Generation: The LLM proposes a surgical fix (e.g., adding try-except blocks or fixing logic).

Verification: The agent runs a "Health Check" (compilation or test execution).

Commitment: If the check passes, the file is updated. If it fails, the agent tries a different approach or exits safely.
