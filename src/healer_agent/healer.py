...
def get_suspect_list(log_content):
    ...
    Includes an Infrastructure Blacklist and a Deep Scan Fallback.
    """
    suspects = []
    
    # 1. DIRECTORY BLACKLIST (Stop the agent from looking in heavy/private folders)
    # Added 'sre_brain' so the agent stops trying to fix its own code!
    IGNORE_DIRS = {'.git', 'venv', 'target', 'node_modules', 'build', '__pycache__', '.idea', '.vscode', 'sre_brain'}
    
    # 2. FILE BLACKLIST (Infrastructure Guardrails)
    # This prevents the agent from "suicide-patching" the pipeline or messing with lockfiles
    IGNORE_FILES = {'Jenkinsfile', 'healer.py', 'current_build.log', 'package-lock.json'}
    
    # Extensions the agent knows how to fix (for the Deep Scan)
    SOURCE_EXTS = ('.js', '.ts', '.java', '.py', '.cpp', '.h', '.go', '.rs')

    # ==========================================
    # PASS 1: The Targeted Log Scan
    # ==========================================
    for root, dirs, files in os.walk('.'):
        ...
        for file in files:
            # Skip the blacklisted files immediately
            if file in IGNORE_FILES:
                continue
                
            # If the bare filename appears anywhere in the log...
    ...
    # ==========================================
    # PASS 2: The Deep Scan Fallback
    # ==========================================
    # If the log crashed early (like OpenSSL) and mentioned no files, 
    # the agent will proactively grab all source files to audit them.
    if not suspects:
        print("🔍 LOG ANALYSIS: No specific files found in log. Initiating Deep Source Crawl...")
        for root, dirs, files in os.walk('.'):
            dirs[:] = [d for d in dirs if d not in IGNORE_DIRS]
            ...
            for file in files:
                ...
                # Add all valid source and config files
                if file.endswith(SOURCE_EXTS) or file in ['pom.xml', 'package.json']:
                    file_path = os.path.relpath(os.path.join(root, file), '.').replace('./', '')
                    suspects.append(file_path)
...
def get_diagnosis(file_content, error_log, target_file, supporting_context=""):
    """STAGE 1: Now with 100% more peripheral vision and disciplined health checks."""
    url = "http://localhost:11434/api/generate"
    
    prompt = f"""You are an elite Site Reliability Engineer fixing a broken CI/CD build.
    ...
    TASK: Identify if the error is in the current target file, caused by a mismatch with supporting files, or if the target file is completely healthy.
    
    CRITICAL DIRECTIVES:

    1. THE HEALTH CHECK (SAFE EXIT):
    If the provided code for '{target_file}' is syntactically correct and UNRELATED to the specific errors in the log, you MUST respond with exactly: "STATUS: HEALTHY". Do NOT suggest refactors, optimizations, or style changes. Only fix build-breaking errors.

    2. THE TARGET SWITCHER:
    ...
    try:
        # Note: Ensure MODEL is defined in your broader script scope, as it was in your original code
...
def main():
    ...
    for round_num in range(1, max_global_rounds + 1):
        ...
        # ==========================================
        # 🛑 THE VICTORY LAP GATEKEEPER
        # ==========================================
        # If it's past round 1, and none of our suspects are actually mentioned 
        # in the current error log, it means the specific errors are gone!
        if round_num > 1 and not any(s in master_log_content for s in all_suspects):
            print("🎉 VICTORY LAP: No specific file errors left in the log. The build should be clean!")
            break
        ...
        while attempt <= MAX_RETRIES:
            ...
            # ==========================================
            # 🛡️ THE HEALTH CHECK INTERCEPTOR
            # ==========================================
            if "STATUS: HEALTHY" in diagnosis_to_use:
                print(f"✅ CLEAN BILL OF HEALTH: AI confirmed {target_file} is not broken.")
                print("🛑 Skipping repair to prevent hallucinated bugs.")
                
                # Add to patched_files so we don't keep picking it up in Deep Scans
                patched_files.append(target_file) 
                
                # Set to True so the orchestrator cleanly loops back to the start of the next round
                file_fixed = True 
                break
